...
def compute_z(x,W,b):
    ...
    z = ((W @ x) + b).reshape(-1) # reshape to ensure it's a 1D array
...
def compute_a(z):
    ...
    z_shifted = z - np.max(z) # shifts all z values to below zero to prevent overflow
...
def compute_dL_da(a, y):
    ...
    dL_da = np.zeros_like(a) # dL_da is zero for all y except the correct class
...
def compute_da_dz(a):
    ...
    da_dz = np.zeros((a.shape[0], a.shape[0])) # gradient matrix of zeros
...
def compute_dz_dW(x,c):
    ...
    x = x.reshape(-1)
    dz_dW = np.zeros((c, x.shape[0])) # gradient matrix of zeros
...
def compute_dz_db(c):
    ...
    dz_db = np.ones(c) # gradient vector of ones
...
def predict(Xtest, W, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        P[i] = a # probability vector
        Y[i] = np.argmax(a) # index of class with highest probability
